...
if not mt.testCurrentPassword(current):
    ...
    context.plone_utils.addPortalMessage(failMessage, type='error')
    return context.password_form(context,
                                 REQUEST,
                                 error=failMessage)

if password != password_confirm:
    failMessage=_(u'password_mismatch')
    context.plone_utils.addPortalMessage(failMessage, type='error')
    return context.password_form(context,
                                 REQUEST,
                                 error=failMessage)
...
failMessage=context.portal_registration.testPasswordValidity(password, confirm=password_confirm)
if failMessage:
    context.plone_utils.addPortalMessage(failMessage, type='error')
    return context.password_form(context,
                                 REQUEST,
